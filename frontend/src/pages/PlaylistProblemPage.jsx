...
function PlaylistProblemPage() {
    ...
    const removeProblem = async (problemId) => {
        ...
        try {
            ...
            // navigate("/playlist")
            setProblemInPlaylist((prev) =>
            prev.filter((item) => item.problemId !== problemId)
        );
        } catch (error) {
            console.log(error)
        }
  ...
  return (
    (problemInPlaylist && <div className='w-full min-h-screen text-white'>
        <div className='w-full h-full flex flex-col gap-3 mt-4 px-4'>
            ...
            {(problemInPlaylist.problems).map((elem) => (
                ...
                <div key={elem.id} className='bg-[#242424] w-full flex justify-between items-center px-4 py-2 rounded-lg' >
                    {console.log(elem)}
                    ...
                    <span className='w-[50%] flex justify-end items-center gap-2'>
                        ...
                        {!problemInPlaylist.isPaid ? (<span onClick={() => removeProblem(elem.problemId)} className='bg-orange-400 p-1 rounded-2xl cursor-pointer'><Trash stroke='white' fill='white' /></span>) : ""}
                    </span>
                </div>
            ))}
        </div>
